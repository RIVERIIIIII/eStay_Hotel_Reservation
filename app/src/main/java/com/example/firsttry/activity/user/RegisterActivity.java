    ...
    private void RegisterRequest(String account, String photo, String email, String password, String confirmPassword) {
        UserApi.register(account, email, password, confirmPassword, photo, new UserApi.UserCallback() {
            ...
            public void onSuccess(String token) {
                ...
                // 后台执行数据库插入
                new Thread(() -> {
                    dbHelper.insertUser(account, photo, email, token);
                    ...
                    runOnUiThread(() -> {
                        ...
                        try {
                            ...
                            Log.e("RegisterActivity", "Target activity not found", e);
                            Toast.makeText(RegisterActivity.this, "无法跳转：目标页面未找到", Toast.LENGTH_SHORT).show();
                        } catch (Exception e) {
                            Log.e("RegisterActivity", "Navigation failed", e);
                            Toast.makeText(RegisterActivity.this, "页面跳转发生错误", Toast.LENGTH_SHORT).show();
                        }
                    });
                }).start();
            }
