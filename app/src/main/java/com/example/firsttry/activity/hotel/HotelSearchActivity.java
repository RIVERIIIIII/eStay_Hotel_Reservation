...
public class HotelSearchActivity extends AppCompatActivity {
    ...
    private List<HotelModel> featuredHotels = new ArrayList<>();
    private List<String> bannerImageUrls = new ArrayList<>();

    private void setupBanner() {
        // 初始化默认图片
        bannerImageUrls = Arrays.asList(
                "https://via.placeholder.com/1024x512?text=Loading...",
                "https://via.placeholder.com/1024x512?text=Loading...",
                "https://via.placeholder.com/1024x512?text=Loading..."
        );
        
        BannerAdapter adapter = new BannerAdapter(bannerImageUrls);
        adapter.setOnBannerClickListener(position -> {
            // Click to navigate to Hotel Detail
            if (position < featuredHotels.size()) {
                Intent intent = new Intent(HotelSearchActivity.this, HotelDetailActivity.class);
                intent.putExtra(HotelDetailActivity.EXTRA_HOTEL_ID, featuredHotels.get(position).getId());
    ...
    private void loadFeaturedHotels() {
        HotelApi.getFeaturedHotels(new HotelApi.FeaturedHotelsCallback() {
            @Override
            public void onSuccess(List<HotelModel> hotels) {
                if (hotels != null && !hotels.isEmpty()) {
                    ...
                    for (HotelModel hotel : hotels) {
                        newImageUrls.add(hotel.getThumbnailUrl());
                    }
                    bannerImageUrls = newImageUrls;
            ...
            public void onError(String message) {
                Log.e("HotelSearchActivity", "加载推荐酒店失败: " + message);
                Toast.makeText(HotelSearchActivity.this, "加载推荐酒店失败", Toast.LENGTH_SHORT).show();
            }
            
            @Override
            public void onFailure(IOException e) {
                Log.e("HotelSearchActivity", "网络请求失败: " + e.getMessage());
                Toast.makeText(HotelSearchActivity.this, "网络请求失败", Toast.LENGTH_SHORT).show();
            }
        });
    }
